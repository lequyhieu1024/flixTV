...
@section('content')
		...
		<div class="container">
			<div class="row">
				<div class="col-12 col-xl-6">
					<h1 class="section__title section__title--head">Danh mục {{ $movies->name }}</h1>
				</div>
				...
				<div class="col-12">
					<div class="catalog__nav catalog__nav--relative">
						...
						<div class="slider-radio">
							...
							{{-- <input type="radio" name="grade" id="popular"><label for="popular">Viewst</label> --}}
						...
						@foreach ($movies as $item)
							<div class="col-12 col-md-6 col-xl-4">
								<div class="card card--big">
									<a href="{{route('detail/'.$item->movie_id.'/'.convert_slug($item->title))}}" class="card__cover">
										<img style="height: 260px;" src="{{IMG_URL.'thumbnail/'.$item->thumbnail_movie}}" alt="">
									...
									<div class="card__content">
										<h3 class="card__title"><a href="{{route('detail/'.$item->movie_id.'/'.convert_slug($item->title))}}">{{ $item->title }}</a></h3>
										<ul class="card__list">
											<li>{{ $item ->duration_minutes}} phút</li>
											<li>{{ $item ->name}}</li>
											<li>{{ date("Y", strtotime($item->release_date)) }}</li>
										</ul>
										<ul class="card__info">
											<li><span>Lượt xem:</span><span><a href="#">{{ $item->views}}</a></span></li>
										</ul>
										<ul class="card__info">
											<li><span>Đạo diễn:</span><span><a href="#"><li>{{ $item ->author}}</li></a></span></li>
										</ul>
										<p class="card__tagline">{{ $item ->nation}}</p>
									</div>
								</div>
							</div>
						@endforeach
	...
	<section class="section section--pb0 section--gradient">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<h2 class="section__title">Phim mới nhất</h2>
				</div>

				<div class="col-12">
					<div class="section__carousel-wrap">
						<div class="section__carousel owl-carousel" id="newest">
							@foreach ($newestMovies as $item)
								<div class="card">

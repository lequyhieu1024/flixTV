...
<style>
    ...
    <section class="section section--head section--gradient section--details-bg">
		<div class="section__bg" data-bg="{{IMG_URL.'thumbnail/'.$movie->thumbnail_trailer}}"></div>
		<div class="container">
			<!-- article -->
			<div class="article">
				<div class="row">
					...
					<!-- video player -->
					<div class="col-12 col-xl-8">
                        <iframe width="500" height="315" src="{{ $movie->link_movie }}" frameborder="0" allow="autoplay" allowfullscreen></iframe>

						{{-- @php print_r($episodes) @endphp --}}
						
						<div class="">
							@if (empty($episodes))
								<a href="{{ route('detail/'.$movie->id.'/'.convert_slug($movie->title)) }}" class="categories__item col-2 col-md-2 col-lg-2">1</a>
							@elseif (!empty($episodes))
								<a href="{{ route('detail/'.$movie->id.'/'.convert_slug($movie->title)) }}" class="categories__item col-2 col-md-2 col-lg-2">1</a>
								@foreach ($episodes as $item)
									<a href="#" onclick="changeEpisode('{{ $item->link }}')" class="categories__item col-2 col-md-2 col-lg-2">{{ $item->episode }}</a> 
								@endforeach
							@endif
						</div>
						<div class="article__actions article__actions--details">
							<button class="article__favorites" type="button"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path d="M16,2H8A3,3,0,0,0,5,5V21a1,1,0,0,0,.5.87,1,1,0,0,0,1,0L12,18.69l5.5,3.18A1,1,0,0,0,18,22a1,1,0,0,0,.5-.13A1,1,0,0,0,19,21V5A3,3,0,0,0,16,2Zm1,17.27-4.5-2.6a1,1,0,0,0-1,0L7,19.27V5A1,1,0,0,1,8,4h8a1,1,0,0,1,1,1Z"></path></svg>Thêm vào Yêu thích</button>
						</div>
					</div>
					<!-- end video player -->
					...
					<div class="col-12 col-xl-8">
						...
						<div class="categories">
							...
							<a href="#" class="categories__item">{{ $movie->name}}</a>
						</div>
						<!-- end categories -->
						...
						<div class="comments">
							...
							<div class="tab-content">
								...
								<div class="tab-pane fade show active" id="tab-1" role="tabpanel">
										...
										@foreach ($comments as $item)
											<li class="comments__item">
												<div class="comments__autor">
													<img class="comments__avatar" src="{{IMG_URL}}/avatar.svg" alt="">
													<span class="comments__name"> {{$item->username}} </span>
													<span class="comments__time"> {{$item->comment_time}} </span>
												</div>
												...
												{{-- <div class="comments__actions">
													...
													<button class="btn-like" style="color:white">{{ $item->like }}</button>
													<button data-id="{{$item->comment_id}}" type="button"><svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 512 512"><path fill="#74C0FC" d="M313.4 32.9c26 5.2 42.9 30.5 37.7 56.5l-2.3 11.4c-5.3 26.7-15.1 52.1-28.8 75.2H464c26.5 0 48 21.5 48 48c0 18.5-10.5 34.6-25.9 42.6C497 275.4 504 288.9 504 304c0 23.4-16.8 42.9-38.9 47.1c4.4 7.3 6.9 15.8 6.9 24.9c0 21.3-13.9 39.4-33.1 45.6c.7 3.3 1.1 6.8 1.1 10.4c0 26.5-21.5 48-48 48H294.5c-19 0-37.5-5.6-53.3-16.1l-38.5-25.7C176 420.4 160 390.4 160 358.3V320 272 247.1c0-29.2 13.3-56.7 36-75l7.4-5.9c26.5-21.2 44.6-51 51.2-84.2l2.3-11.4c5.2-26 30.5-42.9 56.5-37.7zM32 192H96c17.7 0 32 14.3 32 32V448c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V224c0-17.7 14.3-32 32-32z"/></svg></button>
													<button class="btn-dislike" style="color:white">{{ $item->dislike }}</button>
													<button data-id="{{$item->comment_id}}"  type="button"><svg xmlns="http://www.w3.org/2000/svg" transform="rotate(180)" viewBox="0 0 512 512"><path fill="#74C0FC" d="M313.4 32.9c26 5.2 42.9 30.5 37.7 56.5l-2.3 11.4c-5.3 26.7-15.1 52.1-28.8 75.2H464c26.5 0 48 21.5 48 48c0 18.5-10.5 34.6-25.9 42.6C497 275.4 504 288.9 504 304c0 23.4-16.8 42.9-38.9 47.1c4.4 7.3 6.9 15.8 6.9 24.9c0 21.3-13.9 39.4-33.1 45.6c.7 3.3 1.1 6.8 1.1 10.4c0 26.5-21.5 48-48 48H294.5c-19 0-37.5-5.6-53.3-16.1l-38.5-25.7C176 420.4 160 390.4 160 358.3V320 272 247.1c0-29.2 13.3-56.7 36-75l7.4-5.9c26.5-21.2 44.6-51 51.2-84.2l2.3-11.4c5.2-26 30.5-42.9 56.5-37.7zM32 192H96c17.7 0 32 14.3 32 32V448c0 17.7-14.3 32-32 32H32c-17.7 0-32-14.3-32-32V224c0-17.7 14.3-32 32-32z"/></svg></button>
												</div> --}}
											</li>
										@endforeach
										@if (empty($comments))
											<button style="color:white">Chưa có bình luận</button>			
										@elseif(count($comments) > 1)
											<button data-id="{{$movie->id}}" class="load-more" style="color:white">Xem tất cả bình luận</button>
										@else
									...
									<form action="{{route('send-comment/'.$movie->id.'/'.convert_slug($movie->title)) }}" method="POST" class="comments__form">
										@if (isset($_SESSION['errors']) && isset($_GET['msg']))
											<span style="color:red">{{ $_SESSION['errors']['comment'] }} </span>
										@endif
										<div class="sign__group">
											<textarea id="text" name="comment" class="sign__textarea input_comment" placeholder="Thêm bình luận ..."></textarea>
										</div>
										<button type="submit" class="sign__btn btn-send">
					...
					<div class="col-12 col-xl-4">
						<div class="sidebar sidebar--mt">
							...
							<div class="row row--grid">
								<div class="col-12">
									<h5 class="sidebar__title">Phim mới</h5>
								</div>
								@foreach ($newMovies as $item)
									<div class="col-6 col-sm-4 col-md-3 col-xl-6">
										<div class="card">
											<a href="{{route('detail/'.$item->id.'/'.convert_slug($item->title))}}" class="card__cover">
												<img style="height: 250px" src="{{IMG_URL.'thumbnail/'.$item->thumbnail_movie}}" alt="">
											...
											<h3 class="card__title"><a href="{{route('detail/'.$item->id.'/'.convert_slug($item->title))}}">{{ $item->title }}</a></h3>
											<ul class="card__list">
												<li>{{$item->nation}}</li>
												<li>{{ $item->name}}</li>
												<li>{{ date("Y", strtotime($item->release_date)) }}</li>
											</ul>
										</div>
									</div>
								@endforeach
	...
	<section class="section">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<h2 class="section__title"><a href="{{route('movies-by-genres/'.$movie->genre_id)}}">Những bộ phim cùng thể loại</a></h2>
				</div>
				<div class="col-12">
...
@endsection
@section('script')
	<script src="{{BASE_URL.'public/client/js/ajax-watch-movie.js'}}"></script>
	<script src="{{BASE_URL.'public/client/js/ajax-load-comment.js'}}"></script>
@endsection
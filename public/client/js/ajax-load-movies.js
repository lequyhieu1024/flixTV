function convertSlug(title) {
    ...
    const slug = title.toLowerCase()
        ...
        .replace(/[^\w\-]+/g, ''); // Xóa các ký tự không phải là chữ cái, số, hoặc dấu gạch ngang
    return slug;
}
const viewest = document.querySelector('#Popular');
const favourite = document.querySelector('#Newest');
const newest = document.querySelector('#Featured');
	viewest.addEventListener('click', () => {
		const xhr = new XMLHttpRequest();
		xhr.open('GET', 'get-viewest-movies', true);

		xhr.onload = function() {
			if (this.status === 200) {
				const movies = JSON.parse(this.responseText);
				changeToViewest(movies);
			}
		}
		xhr.send();
		// alert('hihi')
    ...
    newest.addEventListener('click', () => {
        const xhr = new XMLHttpRequest();
        xhr.open('GET', 'get-newest-movies', true);

        xhr.onload = function() {
            if (this.status === 200) {
                const movies = JSON.parse(this.responseText);
                changeToNewest(movies);
            }
        }
	...
	function changeToViewest(movies) {
		const contentChanged = document.querySelector('.change-content');
		let newHTML = '';
		...
		movies.forEach(movie => {
            newHTML += `
                <div class="col-6 col-sm-4 col-lg-3 col-xl-2">
                    <div class="card">
                        <a href="detail/${movie.id}/${convertSlug(movie.title)}" class="card__cover">
                            <img style="height: 280px" src="public/images/thumbnail/${movie.thumbnail_movie}" alt="">
                        ...
                        <h3 class="card__title"><a href="detail/${movie.id}/${convertSlug(movie.title)}">${movie.title}</a></h3>
                        <ul class="card__list">
                            <li>${movie.nation }</li>
                            <li>${movie.name}</li>
	...
	function changeToNewest(movies) {
		const contentChanged = document.querySelector('.change-content');
		let newHTML = '';
